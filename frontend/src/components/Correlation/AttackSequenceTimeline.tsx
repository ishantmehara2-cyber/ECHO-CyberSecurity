...
export const AttackSequenceTimeline = () => {
  const [selectedStage, setSelectedStage] = useState<AttackStageItem>(RECONSTRUCTED_ATTACK_STAGES[0]);
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-mono px-2 py-0.5 bg-cyan-950 text-cyan-400 border border-cyan-800 rounded font-bold">
              CLICK EVENT STAGES FOR REASONING
            </span>
          </div>
        ...
        {RECONSTRUCTED_ATTACK_STAGES.map((stage) => {
          ...
          return (
            <div
              ...
              className={`bg-dark-900 border rounded-xl p-5 space-y-3 transition-all cursor-pointer relative overflow-hidden ${
                isSelected
                  ? 'border-cyan-400 shadow-[0_0_20px_rgba(6,182,212,0.2)] bg-dark-900'
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="md:col-span-2 p-3 bg-cyan-950/40 border border-cyan-900/60 rounded-lg text-xs space-y-1 font-sans">
                  <div className="font-bold text-cyan-300 flex items-center gap-1.5 font-mono text-[11px]">
                    ...
                    <span>WHY CONNECTED? (EXPLAINABLE REASONING)</span>
                  </div>
              ...
              <div className="p-2.5 bg-dark-800 border border-dark-700 rounded-lg text-xs font-sans text-slate-400 flex items-center gap-2">
                <Sparkles className="w-3.5 h-3.5 text-cyan-400 shrink-0" />
                <span>
                  <strong className="text-cyan-300 font-mono text-[11px]">WHY THIS MATTERS:</strong> This event establishes stage {stage.stageNumber} of the reconstructed attack chain using verified timestamp alignment and matching entity attributes.
                </span>
              </div>
            </div>
          );
