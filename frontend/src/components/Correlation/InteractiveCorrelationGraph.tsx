...
export function buildGraphData(customNodes?: any[], customLinks?: any[]): { nodes: GraphNode[]; links: CorrelationLink[] } {
  ...
  // 1. Build Nodes with well-spaced percentages
  const nodes: GraphNode[] = rawNodes.map((n, idx) => {
    const defaultX = 12 + ((idx * 22) % 76);
    const defaultY = 25 + ((idx * 30) % 55);
  ...
  // 2. Build Links & Match Node IDs by label if necessary
  let links: CorrelationLink[] = rawLinks.map((l, idx) => {
    // Attempt matching by ID first
    let srcNode = nodes.find(n => n.id === l.sourceNodeId);
    let tgtNode = nodes.find(n => n.id === l.targetNodeId);

    // If ID match failed, attempt label matching
    if (!srcNode && l.sourceLabel) {
      srcNode = nodes.find(n => n.label.toLowerCase() === l.sourceLabel.toLowerCase());
    }
    if (!tgtNode && l.targetLabel) {
      tgtNode = nodes.find(n => n.label.toLowerCase() === l.targetLabel.toLowerCase());
    }

    // Fallback to indexed nodes
    if (!srcNode) srcNode = nodes[idx % nodes.length] || nodes[0];
    if (!tgtNode) tgtNode = nodes[(idx + 1) % nodes.length] || nodes[nodes.length - 1];

    return {
      id: l.id || `link-${idx + 1}`,
      sourceNodeId: srcNode?.id || 'node-1',
      targetNodeId: tgtNode?.id || 'node-2',
      sourceLabel: l.sourceLabel || srcNode?.label || 'Source',
      targetLabel: l.targetLabel || tgtNode?.label || 'Target',
      matchingFactors: l.matchingFactors || [
        { fieldName: 'Temporal Proximity', value: '< 5 Minutes', description: 'Observed sequentially in telemetry' },
      ...
      humanExplanation: l.humanExplanation || `Connected because ${srcNode?.label || 'Source'} and ${tgtNode?.label || 'Target'} were correlated in telemetry.`
    };
  });

  // 3. If nodes exist but no links exist, create sequential correlation edges between nodes
  if (nodes.length > 1 && links.length === 0) {
    ...
    for (let i = 0; i < nodes.length - 1; i++) {
      links.push({
        ...
        matchingFactors: [
          { fieldName: 'Temporal Sequence', value: 'Observed Order', description: 'Events occurred in sequential order' },
          { fieldName: 'Shared Telemetry', value: 'Evidence Stream', description: 'Common telemetry source' }
        ],
        confidenceScore: 88,
        confidenceLevel: 'HIGH',
        humanExplanation: `Connected because ${nodes[i].label} and ${nodes[i + 1].label} were correlated in the evidence stream.`
      });
    }
  }
...
export const InteractiveCorrelationGraph = ({
  ...
  // Synchronize selection when nodes/links update
  useEffect(() => {
    if (nodes.length > 0) setSelectedNode(nodes[0]);
    if (links.length > 0) setSelectedLink(links[0]);
    ...
    <div className="bg-dark-800 border border-dark-700 rounded-xl p-6 space-y-6 shadow-xl font-sans">
      ...
      {viewMode === 'simplified' ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-dark-900 border border-cyan-900/60 rounded-xl p-6 space-y-4 shadow-2xl relative overflow-hidden min-h-[460px]">
            <div className="flex justify-between items-center text-xs font-mono text-slate-400 border-b border-dark-800 pb-3">
              <span className="flex items-center gap-1.5 text-cyan-400 font-bold">
                <Network className="w-4 h-4" /> Sequential Attack Progression Chain ({primaryPathNodes.length} Entities)
              </span>
            ...
            <div className="space-y-3 py-2">
              {primaryPathNodes.map((node, idx) => {
                ...
                const relatedLink = links.find(l => l.sourceNodeId === node.id || l.targetNodeId === node.id) || links[idx % links.length] || links[0];
